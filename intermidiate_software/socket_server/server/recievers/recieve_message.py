...
def match_checksums(message: bytes, checksum: str) -> bool:
    ...
    message_checksum = zlib.crc32(message)      # Calculating checksum of revieved message

    if message_checksum == int(checksum):       # Matching checksums
        return True
    return False
...
def get_packet(connection) -> tuple[str] | dict:
    ...
    try:
        ...
        error = f'GET_PACKET - DECODING MESSAGE or CHECKSUM ERROR: {e}'
        return {'error': error}
...
def recieve_message(connection) -> str | dict:
    '''Executes the recieving message process'''

    packet = get_packet(connection)                 # Recieving packet
    message = parse_message(packet)                 # Deserializing message

    return {'type': 'RECIEVED', 'message': message} # Returning recieved message