...
def get_packet(client) -> tuple[str] | dict:
    '''Recieves a packet and decodes it, else returns error message'''

    try:
        length: bytes = client.socket.recv(5)                  # Recieving message length
    ...
    try:
        message: bytes = client.socket.recv(decoded_length)    # Recieving message of given length
    except Exception as e:
        error = f'GET_PACKET - RECIEVING MESSAGE ERROR: {e}'
        return {'error': error}

    try:
        checksum: bytes = client.socket.recv(10)               # Recieving checksum
